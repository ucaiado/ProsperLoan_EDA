...
together. The scale of each one is prety different. And how about the ineterest 
paid? The distribution differs for each term? First, I will see the distribution 
of the entire data base:

```{r, echo=FALSE, cache=TRUE, fig.width=14, fig.height=9}
by(d$LoanOriginalAmount, d$Term, summary)
```
...
is someone lending money directly to somebody else, without a financial institution
between them. Given that one of the golden rules when you want to reduce the risk
of an investment  is 
<a href="http://en.wikipedia.org/wiki/Diversification_%28finance%29">diversification</a>,
a peer-to-peer lender should split their money to different borrowers as much a 
possible. 

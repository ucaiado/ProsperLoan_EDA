...
<a href="http://www.investopedia.com/terms/s/securitiesact1933.asp">a piece of legislation</a>
...
and I believe that they were created to comply regulatory rules. In general, they are
...
related to these two factors: risk and return.

There are variable explicit about risk measurements (what was the classification 
when the loan was took, the scores from Prosper), about how much was paid and earned,
...
with term of 36 months have a more dispersed distribution. However, it is difficult
to see how the how loans with term of 12 months are distributed. Bellow are the 
Box plots.
...
function of the risk score. First, let me see the quartiles of the Prosper Rating.
...
a issue, given that  a averate rate of, for instance, 5.3, has a meaning. It tells
me that there is more data point on 5 than in 6. I also notice that there are many
data point without a risk score. Below is a histogram just including those that 
has a score setted:
...
the distribution splitting the data by term, but I will check it later. Maybe the
income can help me understand something:
...
income value is  $ 1,750,000. If I look at the income range, I can see that it was
grouped just until $100,000. Let's look at data where the monthly income less 
then 100,000 and has a prosper score setted.
...
I need to relabel the categories. Also, I am already seeing that there are a lot of categories 
that are less than 2,000 observations. I am goint to rename those to 'Other'
...
#create a new variable with renamed loan status
d$ListingCategory.renamed <- apply(d['ListingCategory..numeric.'],1,rename_listingCat)
...
Much better. The loans to roll over existing debts are predominant. Let't see now how
in debt people are when take loan using Prosper. Given that the loans to debt 
consolidation are most comon, I would expect that the ratio was be greater.
...
This plot does not help. I going to filtering out the debt/income greater then 1.
...
Let's start looking at how risk score and borrower rate relate with each other. I
expect that I find a stringh relatinship between then, after all, the interest rate
should reflect the risk of the loan.
...
loans  with higher risk. What about risk score and montly income ? Is there any 
relationship? I am also filtering out monthly income equal to 0 (zero) in the 
test bellow. 
...
with(subset(d, d$StatedMonthlyIncome < 10000 &
               ...
               !is.na(ProsperRating..numeric.)),
     cor.test(ProsperRating..numeric., StatedMonthlyIncome,
              method = 'pearson'))
```

The test shows that the correlation is significant, given that the p-value is 
lower than 0.05 and indicates that as higer the monthly income, better is the 
risk score. On the other hand, the correlation coefficient, around 0.2, is
not strong. It can be confirmed by the chart below.
...
with(subset(d, d$StatedMonthlyIncome < 10000 &
               d$StatedMonthlyIncome > 0 &
               !is.na(ProsperRating..numeric.)),
     cor.test(ProsperRating..numeric., DebtToIncomeRatio,
              method = 'pearson'))
```
...
of variables, that is not Prosper Rating and Borrower Ratio, that presents a strong
...
# d$ProsperRating.f <- as.factor(d$ProsperRating..numeric.)
# #histogram of income range filtered
# ggplot(aes(y = DebtToIncomeRatio, x =ProsperRating.f ), 
#        data = subset(d, d$DebtToIncomeRatio < 1 &
#                        !is.na(ProsperRating..numeric.))) +
#   geom_boxplot(color = "black")
```

#explore the last chart factoring by year


#explore factorizyng between home owner and non homw owner

#explore prosper rating and listCategory. use gray scale

#also explore deliquencies

#compare the median ineterest rate between years using a  line. MAybe separe 
#between homeowner

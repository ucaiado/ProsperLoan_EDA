...
```{r Number of Loans by Interest binwidth, echo=FALSE, cache=TRUE, fig.width=11, fig.height=7}
#creating new variables
summary(d$BorrowerRate)
...
```{r Number of Loans by Interest binwidth 2, echo=FALSE, cache=TRUE, fig.width=11, fig.height=7}
#creating new variables
ggplot(aes(x = BorrowerRate), data = d) +
...
```{r, echo=FALSE, cache=TRUE, fig.width=14, fig.height=9}
#creating new variables
ggplot(aes(x = LoanOriginationDate), data = subset(d, BorrowerRate==0.3177 | BorrowerRate==0.3199)) +
  geom_histogram() + 
...
All loans of these buckets were 36 months term. Let`s see how the interest rate are
distributed by Term.
...
The most of loans are 36 months Term. It is curious to see that just thos Term 
has a more dispersed distribution. But it is difficult to see how the 12 months 
Term is distributed. Bellow are the Box plots.

```{r Box Plots of BorrowerRate by Term}
...
As the Frequency Polygons showed, 36 months term loans are more dispersed. 



#Write about income range and prosper rating. I can follow the racional and find why this dispersion on 36 month...maybe there are more people with less money borrowing or just more risky.

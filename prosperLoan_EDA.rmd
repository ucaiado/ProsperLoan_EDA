...
I have to tell the truth. Initially, the number of variables intimidated. There are 113,937 rows in this data set and 81 columns (variables). Besides that, I still did not understand the origin of those data very well. It seems to come from Prosper, a <em><a href="http://en.wikipedia.org/wiki/Peer-to-peer_lending">peer-to-peer lending</a></em> marketplace. I have already seen a brazilian website trying to do something similar here in my country, but our "SEC" shut them off. Just finantial companies can lend money here.

The first thing that I should do is understand what kind of variable there are in my hand. After load the data in R, I could see that 20 variables were classified as "factor", 30 was classified as integer and 31 as number. Ok, it doen't add up much, but I know now that I will need to organize mush of my plots using factor variables. Look deeper, I also see that there are 5 variables related to dates and 6 that are just valid to loas originated after 2009. It is curious. After plot the number of loans 
...
ggplot(aes(x = LoanOriginationDate.year), data = d) +
  geom_histogram() + 
  ggtitle('The total of loans by Year')
```

...
It seems that the loans wth term of 36 months are predominant at higher Debt to 
Income ratio. Let me count the occurances to see if it is true.
...
l <- with(filtered_d, by(DebtToIncomeRatio.bucket, Term.f, table) )

l2 <- with(subset(filtered_d, BorrowerRate>0.25),
     by(DebtToIncomeRatio.bucket, Term.f, table) )

l_names <- c("(0,0.14]", "(0.14,0.22]", "(0.22,0.31]", "(0.31,1]",
             "(1,2]", "(2,4]", "(4,6]", "(6,10.2]")

df <- data.frame(matrix(unlist(l), nrow=3))
colnames(df) <- l_names

df2 <- data.frame(matrix(unlist(l2), nrow=3))
colnames(df2) <- l_names

df3 <- df2/df

rownames(df3) <- c(12, 36, 60)
...
#   theme(legend.position = "bottom") +
  geom_boxplot() +
  scale_color_grey() +  
  facet_wrap(~DebtToIncomeRatio.bucket) +
  ggtitle('BorrowerRate by Term and Debt to Income Ratio') 

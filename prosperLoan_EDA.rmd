...
I explore a data set about P2P Loans.  Key goals of the study are to understand
..., and ... .
...
Well, maybe there is something about the amount of the loan and Risk Score. I 
...
It is curious how many loans were taken for Debt Consolidation on all Risk 
...
Well....it is confuse. I am going to break up the Debt To Income Ratio into a
...
Well, if there is this relation with income, maybe there is some relationship 
...
borrower will pay the loan back, you can charge a lower interest rate. 
It is how things work between countries, makes sense that it works in the same 
...
theme_set(theme_minimal(17))
#grouping and summarise data
status_groups <- group_by(filtered_d ,ProsperRating..numeric.)
d.status_groups <- summarise(status_groups,
                             Count = n(),
                             Defaulted = 
                               sum(LoanStatus.renamed=="Chargedoff/Defaulted"))
defaulted_by_risk <- mutate(d.status_groups,
...
Well... I want to check one last thing. Let's see the distribution of borrower 
rate by month. I am also going to plot the default ratio by month and the total 
...
#grouping and summarise data
status_groups <- group_by(filtered_d ,LoanOriginationDate.month)
d.status_groups <- summarise(status_groups,
                             Count = n(),
                             Defaulted = 
                               sum(LoanStatus.renamed=="Chargedoff/Defaulted"))
defaulted_by_month <- mutate(d.status_groups,
                            DefaultedPercent = Defaulted/Count,
                            notDefaulted= 1 - DefaultedPercent)
...
data2 <- melt(defaulted_by_risk[,c('DefaultedPercent')])
...
rate decreases, as well as the percentual Default Rate. 

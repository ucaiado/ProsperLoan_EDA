...
p1 <- ggplot(df, aes(x=row, y=value, fill=variable)) + 
  geom_bar(stat="identity") +
  xlab("\nTerm") +
  ylab("Percentage of Participation\n") +
  scale_fill_grey(start=0.8, end=0.2)+   
...
p2 <- ggplot(df2, aes(x=row, y=value, fill=variable)) + 
  ...
  ggtitle("Percentage of Debt to Income Bucket \non each Term, without smaller DI Ratios")
...
#plot summaries
s_t <- "Percentage of  Defaulted Loans \nby Closed Year And Term \n(D/I < 0.14)"
...
#plot summaries
s<-"Percentual of defaulted loans by Closed Year\n and Term (D/I < 0.14)"
ggplot(aes(x=ClosedDate.year, 
           y=DefaultedPercent,
           group = Term.f),
             data = defaulted_by_term_14) +
...
imagined that years where wereverified a increase in defaulted ratio, the 

...
loans  with higher risk (smaller scores). Let me see the correlation between these two variables.
...
I am going to 
check the Borrower distribution over each listing category bellow.

```{r, echo=FALSE, cache=TRUE}
#set theme
theme_set(theme_minimal(17))
#plot boxplo
ggplot(aes(x = IsBorrowerHomeowner, y =BorrowerRate), data = filtered_d ) +
  scale_color_brewer(type = 'div') +
  geom_boxplot(outlier.size = 0, color = "black") +
  ggtitle('BorrowerRate by Listing Category')
```
...
how long the crdit taker was employed when 
took the loan. Firt, I will check 
```{r, echo=FALSE, cache=TRUE}
#set theme
theme_set(theme_minimal(17))
#plot boxplo
ggplot(aes(x = EmploymentStatus, y =BorrowerRate), data = filtered_d ) +
  scale_color_brewer(type = 'div') +
 geom_boxplot(outlier.size = 0, color = "black") 
  
  
ggplot(aes(x = AvailableBankcardCredit, y =BorrowerRate), data = filtered_d ) +
  scale_color_brewer(type = 'div') +
  geom_point(alpha=1/10) 


table(filtered_d$AmountDelinquent)

with(filtered_d, cor.test(ProsperRating..numeric., CurrentCreditLines,
                          method = 'pearson'))


ggplot(aes(x = AmountDelinquent, y =BorrowerRate), data = filtered_d ) +
  scale_color_brewer(type = 'div') +
  geom_point(alpha=1/10) 
```
```

...
Income ratio. It doesn't necessarily mean something. Let me see how many loans
are at at each bucket, divided by term. I am going to plot all buckets in one 
figure and just the worst debt to income buckets in another one.
...
#grouping and summarise data
term_groups <- group_by(subset(filtered_d, !is.na(DebtToIncomeRatio)) ,Term.f)
d.di_by_term <- summarise(term_groups,
                       CountTotal = n(),
                       count_1 = sum(DebtToIncomeRatio.bucketN ==1),
                       count_2  = sum(DebtToIncomeRatio.bucketN ==2),
                       count_3  = sum(DebtToIncomeRatio.bucketN ==3),
                       count_4  = sum(DebtToIncomeRatio.bucketN ==4),
                       count_5  = sum(DebtToIncomeRatio.bucketN ==5),
                       count_6  = sum(DebtToIncomeRatio.bucketN ==6),
                       count_7  = sum(DebtToIncomeRatio.bucketN ==7),
                       count_8  = sum(DebtToIncomeRatio.bucketN ==8))
#Calculate the percentual participation
di_by_term <- mutate(d.di_by_term,
                     '(0,0.14]' = count_1/CountTotal,
                     '(0.14,0.22]' = count_2/CountTotal,
                     '(0.22,0.31]' = count_3/CountTotal,
                     '(0.31,1]' = count_4/CountTotal,
                     '(1,2]' = count_5/CountTotal,
                     '(2,4]' = count_6/CountTotal,
                     '(4,6]' = count_7/CountTotal,
                     '(6,10.2]' =count_8/CountTotal)


data <- melt(di_by_term[,c('(0,0.14]','(0.14,0.22]','(0.22,0.31]','(0.31,1]',
                            '(1,2]','(2,4]','(4,6]','(6,10.2]')])
row <- factor(rep(c(12,36,60),length=24), c(12,36,60))
df <- cbind(data, row)

#Calculate the percentual participation
di_by_term2 <- mutate(d.di_by_term,
                     '(1,2]' = count_5/(count_5+count_6+count_7+count_8),
                     '(2,4]' = count_6/(count_5+count_6+count_7+count_8),
                     '(4,6]' = count_7/(count_5+count_6+count_7+count_8),
                     '(6,10.2]' =count_8/(count_5+count_6+count_7+count_8))

#reshape data to plot
data2 <- melt(di_by_term2[,c('(1,2]','(2,4]','(4,6]','(6,10.2]')])
row <- factor(rep(c(12,36,60),length=12), c(12,36,60))
df2 <- cbind(data2, row)


#plot a stacked bar chart
p1 <- ggplot(df, aes(x=row, y=value, fill=variable)) + 
  geom_bar(stat="identity") +
  xlab("\nTerm") +
  ylab("Percentual Participation\n") +
  scale_fill_grey(start=0.8, end=0.2)+   
  ggtitle("Percentual of Debt to Income Bucket \non each Term")

p2 <- ggplot(df2, aes(x=row, y=value, fill=variable)) + 
  geom_bar(stat="identity") +
  xlab("\nTerm") +
  ylab("Percentual Participation\n") +
  scale_fill_grey(start=0.6, end=0.2)+   
  ggtitle("Percentual of Debt to Income Bucket \non each Term, without smaller DI Ratios")
...
distributions of the Borrower rate by Term and filtering out the D/I smaller 
than 0.14. I am also going to facet by year to see if the behaviour changes over 
time.
...
ggplot(aes(x = Term.f, y = BorrowerRate), 
  ...
  ggtitle("Borrower Rate by Term and Year (D/I < 0.14)")

```

Interesting. The dispersion is not related to the D/I ratio, given that it is 
true even in Debt To Income Ratios smaller than 0.14. It remains true at almost 
every year where the Prosper Score were set (after 2008). Just in 2014 this 
relation was not verified.

Let me plot the defaulted ratio by Term to see if it sheds some light on what 
makes the 36 months more dispersed. I am supposing that the default rate affects
the Borrower rate using a moving window. I mean, if the defaulted ratio is high 
this month, but it was low in the "n" last months, it doesn't affect very much 
the interest rate. For another side, after 12 months of high defaulted ratio, 
for sure that the impact will be much bigger in the interest rate. 

I am going to faceted the next chart by year of ClosedDate and filter the loans 
with D/I rate smaller than 0.14 because it accounts for 55% of the overall data. 
...
This chart shows that the loans with Term of 60 months started with a smaller 
defaulted ratio than the loans with Term of 36 months in 2011, what changed just
in 2013. 

As the default ratio was becoming higher, the borrower rate was growing and 
becoming more disperse. Apparently,  always reacting more strongly a year later.
Finally, borrowers rates with term of 60 months sharply increased in 2014 after 
its defaulted ratio becoming higher than the ratio of terms of 36 months in 2013. 

It would explain why the 36 months are more dispersed than the 60 months overall.
For me, it is due to the loans with longer terms have shown a smaller defaulted 
ratio in early stages and the borrower rate just react after some time. 
The recent increase in the defaulted ration will only affect the borrower rate 
more strongly the next years. 

I can't really declare what I am hypothesizing here based just on this chart, 
but so far it seems plausible to me. I believe that If this data set was longer, 
probably I would find a much different histoty.
...
I am going to break up the Debt To Income Ratio into a
categorical one and plot the two variables again. The most of classes I have 
defined as the quartiles of the data.
...
As the number of loans by month increases, the median of interest 
rate decreases, as well as the percentage Defaulted ratio. Right now, I can't 
say why , although I can think in a reason for this behaviour. For instance, in 
finance, if you want to reduce the risk of your investments, you should 
<a href="http://en.wikipedia.org/wiki/Modern_portfolio_theory#Diversification">
...
success. Then, Idecided to make some assumptions and try to check them in the 

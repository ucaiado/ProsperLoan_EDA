...
off the amount of data on each status as percentage. As can be seem bellow, 
11% of the data is related to defaulted or chargedoff loans:
...
close look at the loan original amount splitted by duration. I had to convert 
the variable "Term" to factor to make the chart below:
...
together. The distribution of each one is pretty different. Well, and the 
interest rate? The distribution differs at each term? First, let???s see the 
histogram of the interest paid:
...
less than 100,000 and has a prosper score setted.
...
ggplot(aes(y = BorrowerRate, x = ProsperRating..numeric.), 
       data = subset(d, d$StatedMonthlyIncome < 10000 &
                       !is.na(ProsperRating..numeric.))) +
  geom_point()
####Excluir esse grafico e arrumar textos
```
...
###ARRUMAR. O que eu pensava e o que o boxplot mostrou. Talvez inverter o teste
###de correcalao de o grafico. Vou precisar mexer no texto
```
...
#Calculate the participation as percentage
...
```{r new_14, echo=FALSE, cache=TRUE, fig.height=9}
#set theme
theme_set(theme_minimal(14))
...
The next one, I choose the plot of Borrower Rate by Term of the loan and the 
year of origination.
...
this chart helped me understand the dynamic of the borrowers rates changed 
...
loans by the year when the loans were closed, divided them by term in months. 
...
I changed the way I enconded to bars...

Here I decided to use the year when the loans were closed because I have 
imagined that years where were verified a increase in defaulted ratio, the 
borrower rates probably would  increase, what I believe that can be verified at
the chart about borrowers rate by year.
...
The reasoning and findings from each plot are explained concisely with appropriate variable transformations, other plot decisions, and/or statistics. The text about each figure is descriptive and adds information that the graphic itself would not easily explain
